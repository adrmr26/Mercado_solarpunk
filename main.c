...
void generar_productos(Comuna* comuna, char nombre_productos[][]) {
    int i;
...
void* inicializar_persona(void* arg) {
    ...
    while (1) { //De esta forma el loop es infinito hasta que se encuentre un break 
        int producto_id = rand() % comuna->num_productos; //Se le asigna un producto de la comuna de forma aleatorea
        ...
        int tiempo = rand() % 160 + 1; //Cantidad de tiempo que tarda en minutos  

        if (accion == 1) {
            sem_wait(&mutex_anaqueles); //AQUI SEMAFORO
            comuna->productos[producto_id].disponibilidad += cantidad;
            ...
            sem_wait(&mutex_anaqueles); //AQUI SEMAFORO
            comuna->productos[producto_id].necesidad += cantidad;
...
int main() {
    ...
    for (i = 0; i < Numero_comunas; i++) {
        generar_productos(&comunas[i]);
        for (j = 0; j < Max_numero_personas; j++) {
